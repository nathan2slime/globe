import { Color, DoubleSide, RepeatWrapping, Side } from 'three';
...
async function SetupGlobe(viewer: Viewer, _uniforms: BaseCustomUniforms) {
    ...
    const globeMat = await mad.NewMaterial(SL.vertex.GLOBE, SL.frag.GLOBE_BASE, uniforms);
    const atmMat = await mad.NewMaterial(SL.vertex.GLOBE, SL.frag.GLOBE_ATMOSPHERE, uniforms);
